...
func TestGenvmsnap(t *testing.T) {
	...
	// watch.Error isn't attempted here for the same reason csi_test.go
	// skips it for TestGensnap: watchtools.Until uses a RetryWatcher that
	// would end up retrying forever.
}
...
func TestDiskSnapshots(t *testing.T) {
	...
	t.Run("correlates each disk to its volumesnapshot and original pvc", func(t *testing.T) {
		...
		disks, err := k.diskSnapshots(t.Context(), vmsFor("content1"))
		require.NoError(t, err)
	...
	t.Run("missing volumesnapshot propagates the api error", func(t *testing.T) {
		...
		_, err := k.diskSnapshots(t.Context(), vmsFor("content1"))
		require.ErrorContains(t, err, "snap-disk0")
	})

	t.Run("no content name on the virtualmachinesnapshot", func(t *testing.T) {
		vms := &snapshotv1beta1.VirtualMachineSnapshot{
			ObjectMeta: metav1.ObjectMeta{Name: "vmsnap1", Namespace: "ns"},
			Status:     &snapshotv1beta1.VirtualMachineSnapshotStatus{},
		}
		k := &k8s{kubevirtClient: kubevirtfake.NewSimpleClientset()}

		_, err := k.diskSnapshots(t.Context(), vms)
		require.ErrorContains(t, err, "has no content")
	})

	t.Run("content not found propagates the api error", func(t *testing.T) {
		k := &k8s{kubevirtClient: kubevirtfake.NewSimpleClientset()}

		_, err := k.diskSnapshots(t.Context(), vmsFor("missing"))
		require.Error(t, err)
	})
}
